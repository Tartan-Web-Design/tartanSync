...
localSubPath=(
    ...
    "tartan-live/app/public/wp-content/" 
    "wild/app/public/wp-content/"
...
#   Copies the target folder that is about to be rsynced over to the backup foler (*_bak)
            ...
            ssh  $server rm -R "${thisServerPath%/}_bak"
            ssh  $server rsync --owner --group --archive --compress --delete -eh "${thisServerPath}" "${thisServerPath%/}_bak"
            echo PUSHING...
...
# NB $run, which toggles between a --dry-run and a full run.
        ...
        rsync $run --owner --group --archive  --compress --delete -e  "ssh" --stats "${thisLocalPath}${localSubPath[$environmentIndex]}" "$server:${thisServerPath}"

# chown make the pushed files available to Plesk.  Without this step, everything's borked.

        echo Running Chown on Destination...
        ssh $server chown -R "${websiteChown[$environmentIndex]}" "${thisServerPath}"
...
# NB $run, which toggles between a --dry-run and a full run.
        ...
        rsync $run --owner --group --archive --compress --delete  -e  "ssh" --stats "$server:${thisServerPath}" "${thisLocalPath}${localSubPath[$environmentIndex]}" 
    fi
...
# If all the required parameters aren't passed in, return the "usage" help as output
    ...
    if [[ $r == "run" ]]; then
        ...
        if [ $p == "push" ]; then
            ...
            sanity_check
            ssh $server rm -R "${thisServerPath%/}"
            ssh $server mv "${thisServerPath%/}_bak" "${thisServerPath%/}"
            ssh $server chown -R "${websiteChown[$environmentIndex]}" "${thisServerPath}"
